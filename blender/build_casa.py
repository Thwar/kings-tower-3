...
def panel_door(name, x, z, rot, w=0.85, room="hall", glass=False, y0=0.0):
    ...
    px, pz = P(0, 0)
    box(f"{name}", (px, y0 + 1.05, pz), (w, 2.1, 0.045), M["mahogany"], "Furniture", bevel=0.004, rot_z=rot, part="furniture", room=room)
    for k, (y, h) in enumerate([(y0 + 0.55, 0.7), (y0 + 1.5, 0.9)]):
    ...
    px, pz = P(0, -0.035)
    box(f"{name}_arch", (px, y0 + 2.0, pz), (w - 0.1, 0.05, 0.01), M["mahogany"], "Furniture", bevel=0.003, rot_z=rot, rot_x=0, part="furniture", room=room)
